...
getfloorList, editfloorList ,crefloorList ,delfloorList,
 ...
 uploadusageofFloor,getusageofFloorfiles
  } from '@/api/building'
...
export default {
  ...
  methods: {
    ...
    getusageofFloorfiles(usageofFloorid){ //取得門牌檔案
      var _originFiles = []
      getusageofFloorfiles(usageofFloorid).then(response=>{
        console.log('getusageofFloorfiles=>'+JSON.stringify(response))
        response.result.forEach( item => {
    ...
    handleBlockOption(index, content) { //管委會、廠商資料相關操作
      ...
      if (index === 'update') {
        if(this.activeName === 'MC'){
          ...
          this.$api.building.apiPatchCCommittee(content).then(response => {
            this.$message('更新成功')
            this.getManagementList()
          })
    ...
    handleFloorTableOption(index, content) { //樓層門牌相關操作
      ...
      if (index === 'update') {
        ...
        this.getusageofFloorfiles(content)
          ...
          uploadusageofFloor(content.id,this.id,formData).then(response=>{
