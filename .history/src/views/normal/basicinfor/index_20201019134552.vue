  ...
  methods: {
    ...
    getbuInfo() {
      getbuInfo(this.id,this.buildingid).then(response => {
        ...
        this.crefloor("down",-(response.result[0].floorsOfUnderground))
        this.floor = [ response.result[0].floorsOfAboveGround , response.result[0].floorsOfUnderground , '0']
      })
    },
    crefloor(index,val){
      var _array = []
      var _start = index == "down" ? -1 : 1
      for(var i=_start;i<=val;i++)
      {
    ...
    getFloorList(){
      this.floorData = []
      // let floor = { floors :  this.selectfloor.toString() }
      // crefloor(this.buildingid,floor).then(response =>{
      //       console.log('ok=>'+JSON.stringify(response))
      //   }).catch(error=>{
      //       if(error.response.data.errorMessage == "0x06E0"){
