...
<script>
import { uploadbuildinginfo,downloadbufile,delbufile  } from '@/api/building'
  ...
  methods: {
    ...
    onSubmit() { 
      ...
      this.$refs.form.validate(valid =>{
        if(valid){
          this.$api.building.apiPatchBuildingInfo({uid:this.id,data:this.form}).then(response => {
              this.$message('更新成功')
              this.type = 'view'
              this.getbuInfo()
          })
    ...
    getbuInfo() {
      this.$api.building.apiGetBuildingInfo({uid:this.id,bid:this.buildingid}).then(response=>{
        this.refreshUser()
    ...
    delbufile(fileid){
      this.$api.building.apiDeleteFiles({fid:fileid}).then(response =>{
        this.$message('刪除成功')
        this.getbufiles()
      })
