...
<script>
import { uploadbuildinginfo  } from '@/api/building'
  ...
  methods: {
    ...
    onSubmit() { 
      ...
      this.$refs.form.validate(valid =>{
        if(valid){
          this.$api.building.apiPatchBuildingInfo({data:this.form}).then(response => {
              this.$message('更新成功')
              this.type = 'view'
              this.getbuInfo()
          })
    ...
    handleUploadOption(index,file){
      ...
      this.$api.files.apiPostBuildingFiles({data:this.formData}).then(response=> {
          this.$message('上傳成功')
          this.getbufiles()
      })
      // uploadbuildinginfo({data:this.formData}).then(response => {
      //   this.$message('上傳成功')
      //   this.getbufiles()
      // }).catch(error =>{
