...
export default {
  ...
  methods: {
    ...
    onSubmit() {
      this.$set(this.form,'ownerId',this.form.ownerId)
      this.$set(this.form,'fireManagerId',this.form.ownerId)
      console.log('join=>'+this.form.ownerId.join(','))
      ...
      if(this.form.fireManagerId.length > 1){
        var str = this.form.fireManagerId[0] +','
        for(var i =1;i<this.form.fireManagerId.length;i++){
          str = str.concat(this.form.fireManagerId[i])+','
        }
        str = str.substring(0,str.length-1)
        this.$set(this.form,'fireManagerId',str)
      }
      var _array = []
