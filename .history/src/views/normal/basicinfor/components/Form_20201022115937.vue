...
import { apiGetBuildingInfo  } from '@/api/build'
...
export default {
  ...
  methods: {
    ...
    getbuInfo() {
      apiGetBuildingInfo({uid:this.id,bid:this.buildingid}).then(response=>{
        console.log('newgetbuInfo=>'+JSON.stringify(response))
        this.refreshUser()
    ...
    getbufiles(){
      this.originFiles = []
      getbufiles(this.buildingid).then(response =>{
        console.log('getbufiles=>'+JSON.stringify(response))
