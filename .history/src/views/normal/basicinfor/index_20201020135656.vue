...
export default {
  ...
  data() {
    return {
      ...
      floorConfig: [
          ...
          { label:'備註' , prop:'note',type:'string',mandatory:false},
          { label:'所有人資料' , prop:'linkOwners',format:'ownerselect', mandatory:false, message:'請選擇用戶',trigger:'change'},
          { label:'使用人資料' , prop:'linkUsers',format:'userselect', mandatory:false, message:'請選擇用戶',trigger:'change'} ],
      floorData:[],
      buttonsName: ['編輯','刪除'],
      selectData:[],
      userselectData:[],
  ...
  methods: {
    ...
    getselectOption(){ //取得大樓的廠商分類
      this.options = []
      getselectOption(this.buildingid).then(response => {
        console.log('getselectOption=>'+JSON.stringify(response))
        response.result.forEach(item => {
              let _array = { 
                  id : item.id, 
                  label: item.textName, 
    ...
    getfloors(){
      getfloors(this.buildingid).then(response =>{
        console.log('rr=>'+response.result.length)
      }).catch(error =>{
        console.log('error=>'+error)
      })
    },
    ...
    handleFloorTableOption(index, content) {
      ...
      if (index === 'update') {
        ...
        crefloorList(this.buildingid,content).then(response => {
    ...
    handleRangeOption(content){
      this.getfloorList(content)
    },
    getfloorList(content){
      this.floorData = []
      getfloorList(this.buildingid,content).then(response => {
