...
import { getbuInfo, getreportList, editreportList, crereportList, delreportList } from '@/api/building'
...
export default {
  ...
  methods: {
    ...
    async reportList() {
      try {
        getreportList(this.buildingid).then( response => {
          let mockList = response.result.filter(item => {
            if (item.class !== 'ReportPublic') return false
            return true
          })
          this.blockData = mockList.sort((x,y) => y.isImproved - x.isImproved)
        }).catch(error => {
          console.log(error)
        })
      } catch (error) {
    ...
    handleBlockOption(index, content) {
      ...
      if (index === 'update'){
        editreportList(JSON.stringify(content)).then(response=>{
          this.$message('更新成功')
          this.reportList()
        }).catch(error=>{
          console.log(error)
        ...
        content.class = 'ReportPublic'
        crereportList(this.buildingid,JSON.stringify(content)).then(response=>{
          this.$message('新增成功')
          this.reportList()
        }).catch(error=>{
          console.log(error)
        ...
        delreportList(content).then(response=>{
          this.$message('刪除成功')
          this.reportList()
        }).catch(error=>{
          console.log(error)
