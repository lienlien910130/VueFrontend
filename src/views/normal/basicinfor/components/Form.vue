  ...
  <div>
    ...
    >
      ...
      <el-form-item label="層數" prop="floorsOfAboveGround">
        <span
          >地上 {{ form.floorsOfAboveGround }} 層，地下
          {{ form.floorsOfUnderground }} 層</span
        >
      </el-form-item>
      ...
      <el-form-item label="所有權人" prop="linkOwners">
        <span
          @click="openUser(form.linkOwners)"
      ...
      <el-form-item label="防火管理人" prop="linkFireManagers">
        <span
          @click="openUser(form.linkFireManagers)"
          style="color: #66b1ff; cursor: pointer"
        >
...
export default {
  ...
  methods: {
    ...
    onEdit() {
      ...
      this.linkOwners = [];
      this.form.linkOwners.forEach((item) => {
        this.linkOwners.push(item.id);
      });
      this.linkFireManagers = [];
      this.form.linkFireManagers.forEach((item) => {
        this.linkFireManagers.push(item.id);
      });
    ...
    async postBuildingInfo() {
      var array = [];
      this.linkOwners.forEach((item) => {
        array.push({ id: item.id == undefined ? item : item.id });
      });
      this.form.linkOwners = array;
      array = [];
      this.linkFireManagers.forEach((item) => {
        array.push({ id: item.id == undefined ? item : item.id });
      });
