        ...
        >
          ...
          <div
            :class="[
              'flow-content',
              'grid',
              dragMove.isDown ? 'flow-grab' : null,
            ]"
            :style="{
...
export default {
  ...
  methods: {
        ...
        this.$confirm(
          ...
          .then(() => {
            this.data.nodeList = this.data.nodeList.filter((item) => {
              return item.nodeId !== this.activeElement.nodeId;
            });
            this.$nextTick(function () {
    ...
    async saveFile() {
      ...
      const fileContent = new File([str], this.processId + ".txt", {
        type: "",
      });
      ...
      if (isOk) {
        ...
        lineList.forEach((item) => {
          var indexline = this.processLineArray.findIndex(
            (obj) => obj.optionId === item.id
          );
          if (indexline == -1) {
            addLineList.push({
            ...
            var lineName =
              this.processLineArray[indexline].name !== undefined
        ...
        nodeList.forEach((item) => {
          var indexnode = this.processNodeArray.findIndex(
            (obj) => obj.nodeId === item.nodeId
          );
          if (indexnode == -1) {
            ...
            var processItem = {
              ...
              message:
                this.processNodeArray[indexnode].message == undefined
                  ? ""
                  : this.processNodeArray[indexnode].message,
              nextCpId:
                this.processNodeArray[indexnode].nextCpId == undefined
                  ? ""
                  : this.processNodeArray[indexnode].nextCpId,
              linkRoles: this.processNodeArray[indexnode].linkRoles.sort(
                (x, y) => x.id - y.id
              ),
              linkAccountList: this.processNodeArray[
                indexnode
              ].linkAccountList.sort((x, y) => x.id - y.id),
            };
        ...
        this.processNodeArray.forEach((item) => {
          var indexnode = nodeList.findIndex(
            (obj) => obj.nodeId === item.nodeId
          );
          if (indexnode == -1) {
