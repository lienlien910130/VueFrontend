...
export default {
  ...
  methods: {
    ...
    async handleBlock(title, index, content) {
      ...
      // }
        ...
        if (classArray.length) {
          this.$router.push({
            path: "/process/index",
            query: { l: this.marshallingList.getID() },
          });
        } else {
    ...
    async handleDialog(title, index, content) {
      //Dialog相關操作
      console.log(title, index, JSON.stringify(content));
      if (title === "selfDefenseFireMarshallingMgmt") {
        //消防編組細項
    ...
    async handleMarshallingMgmt(index, content) {
      ...
      if (index === "empty") {
        ...
        var result = await SelfDefenseFireMarshallingMgmt.postMany(
          this.marshallingList.getID(),
          content
        );
